...
8. Тесты: если уже написаны (skill `unit-test`) — не трогать. Иначе сервис + контроллер по правилу `tests.mdc` (`AbstractTest`, без `@WebMvcTest` и Mockito).
9. Прогнать `./gradlew test`, починить красное. Тесты не ослаблять.
